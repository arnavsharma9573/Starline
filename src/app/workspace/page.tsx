      ...
      const processNextCheckpoint = (index = 0) => {
        ...
        if (index < totalCheckpoints) {
          ...
          if (activeMode === "code") {
            ...
            setTimeout(() => {
              // --- YAHAN LOGIC ADD KAREIN ---
              if (activeMode === "plus") {
                // 'Plus' mode ke liye naya data set karein
                ...
                // 'Base' mode (ya koi aur) ke liye puraana data
                      ...
                      <div className="space-y-1 max-h-32 overflow-y-auto pr-1 scrollbar-thin">
                        {mockExistingProjects.length > 0 ? (
                          mockExistingProjects.map((proj) => (
                            <button
                              key={proj.id}
                              onClick={() => handleSelectExistingProject(proj)}
          ...
          {activeMode === "code" && (
            <>
              ...
              {(processingState === "processing" ||
                ...
                <ComplianceRiskMeter
                  score={
                    processingState === "complete" ? finalRiskScore : riskScore
                  }
                  isProcessing={processingState === "processing"}
                  processedCheckpoints={processedCheckpoints}
                  totalCheckpoints={totalCheckpoints}
                  reportUrl={processingState === "complete" ? reportUrl : null}
                />
              )}
            </>
          )}
